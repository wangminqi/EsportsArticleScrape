...
def refine_content(response):
    ...
    target_1 = re.sub(
        '<table class="table2">[\s\S]*?<div class="lt_con">', '', target_1)
                ...
                if 'src="' in i:
                    ...
                    try:
                        ...
                        document.add_picture(
                            path + image_name, width=Inches(5))
                        ...
                        document.add_paragraph(
                            '图片有效但格式特殊，请手动插入 ' + image_name + '!!!!!!!!!!!!!!!!!!!')
# 增加OSError以判断是否爬取非图片资料
                    except OSError:
                        document.add_paragraph(
                            '文章中可能包含视频，请检查 ' + image_name + '!!!!!!!!!!!!!!!!!!!')
                else:
                    # 不包含图片链接，则直接写入word
